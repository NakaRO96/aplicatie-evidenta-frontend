...
import { useAuth } from '../context/AuthContext'; // Pentru a accesa tokenul de admin

function CreateAccountPage() {
  const [name, setName] = useState('');
  const [phoneNumber, setPhoneNumber] = useState('');
  const [password, setPassword] = useState(''); // Pentru testare, putem seta o parolă inițială. În producție, backend-ul ar genera-o.
  const [role, setRole] = useState('client');
  const [message, setMessage] = useState('');
  const [error, setError] = useState('');
  const navigate = useNavigate();
  const { logout } = useAuth(); // Pentru a gestiona erorile de autentificare
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post('http://localhost:5000/api/auth/register', {
        name,
        phoneNumber,
        password, // ATENȚIE: În producție, această parolă ar trebui GENERATĂ de backend și trimisă prin SMS.
        role
      });
      setMessage(res.data.msg);
      alert('Contul a fost creat cu succes! Parola inițială este cea introdusă. (În producție, se trimite prin SMS)');
      // Curăță formularul
      setName('');
      setPhoneNumber('');
      setPassword('');
