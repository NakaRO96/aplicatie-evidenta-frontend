...
function CourseTimer({ userId, onSimulationSaved, onCancel }) {
    ...
    const [finalTotalTime, setFinalTotalTime] = useState(null);
    const [totalPenaltySeconds, setTotalPenaltySeconds] = useState(0);
    const [finalJavelinTime, setFinalJavelinTime] = useState(null);
    ...
    const calculateFinalTime = (raw, currentPenalties) => {
        const penaltySecs = currentPenalties.length * penaltySeconds;
        const finalSecs = raw + penaltySecs;
        return { penaltySecs, finalSecs };
    };

    const startTimer = () => {
        if (!userId) {
            toast.error("Eroare: Niciun utilizator selectat pentru simulare.");
            return;
        }
        if (timerRunning) {
    ...
    const stopTimer = () => {
        ...
        setRawTime(elapsedSeconds);

        if (javelinTimerActive && javelinStartTimeRef.current) {
            const javelinElapsed = (Date.now() - javelinStartTimeRef.current) / 1000;
            setFinalJavelinTime(javelinElapsed);
            setJavelinTimerActive(false);
        }

        setIsStopped(true);
        toast.info("Cronometrul a fost oprit. Acum poți ajusta rezultatele înainte de a le salva.");
    };
    ...
    const resetAll = () => {
        ...
        setRawTime(0);
        setFinalTotalTime(null);
        setTotalPenaltySeconds(0);
        setFinalJavelinTime(null);
    ...
    const handleSaveResults = async () => {
        ...
        try {
            ...
            const { penaltySecs, finalSecs } = calculateFinalTime(rawTime, penalties);

            const payload = {
                user: userId,
                rawTime: rawTime,
                penaltyTime: penaltySecs,
                totalTime: finalSecs,
            ...
            console.error('Eroare la salvarea rezultatului:', error.response ? error.response.data : error.message);
            toast.error('A apărut o eroare la salvare. Verifică log-urile.');
        } finally {
            setIsSaving(false);
        }
    };
    
    // Alte funcții rămân neschimbate
    ...
    const addPenalty = (obstacle) => {
        ...
        if (penalties.includes(obstacle)) {
            const updatedPenalties = penalties.filter(item => item !== obstacle);
    ...
    const addEliminated = (obstacle) => {
        ...
        if (eliminatedObstacles.includes(obstacle)) {
            const updatedEliminated = eliminatedObstacles.filter(item => item !== obstacle);
            setEliminatedObstacles(updatedEliminated);
    ...
    // Efect pentru a actualiza timpul total și penalizările
    useEffect(() => {
        const { penaltySecs, finalSecs } = calculateFinalTime(rawTime, penalties);
        setTotalPenaltySeconds(penaltySecs);
        setFinalTotalTime(finalSecs);
    }, [rawTime, penalties]);
                ...
                <div className="sticky top-0 z-50 bg-white p-2 shadow-md rounded-b-lg -mx-4 sm:-mx-6 lg:-mx-8 mb-4">
                    ...
                    <div className="flex justify-center gap-2 mt-2">
                        ...
                        <button
                            ...
                            disabled={javelinTimerActive || !timerRunning || isStopped}
                        >
                ...
                <div className="bg-blue-50 p-6 rounded-xl shadow-inner border border-blue-200">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-gray-800">
                        <p className="text-lg font-medium">
                            <span className="font-semibold text-blue-700">Timp fără penalizări:</span> {formatTime(rawTime)}
                        </p>
                        <p className="text-lg font-medium">
                            <span className="font-semibold text-blue-700">Timp total cu penalizări:</span> {formatTime(finalTotalTime)}
                        </p>
                        <p className="text-lg font-medium">
                            <span className="font-semibold text-blue-700">Timp Jaloane:</span> {javelinTimerActive ? 'în curs...' : (finalJavelinTime !== null ? formatTime(finalJavelinTime) : '-')}
                        </p>
                        <p className="text-lg font-medium">
                            <span className="font-semibold text-blue-700">Penalizări:</span> {penalties.length} x {penaltySeconds}s = {totalPenaltySeconds}s
                        </p>
